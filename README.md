...
- [ ] **Still needed from you** (I don't have these credentials and won't fabricate them): `ANTHROPIC_API_KEY` and `RESEND_API_KEY` as function secrets (`supabase secrets set ANTHROPIC_API_KEY=... RESEND_API_KEY=...`) — without them, Kairos Mind and application/interview emails will error at runtime, same as before.
