...
- ✅ **KYC document verification** — the identity step is now a guided capture (`IdentityCapture.tsx`: document upload + a live selfie via `getUserMedia`, both required) instead of a plain file input, plus an automated cross-check: `verify-identity-document` reads the uploaded ID with Claude Vision and compares the visible name/address against what the applicant typed (a new `address` field on the application), surfacing a match/mismatch signal on the applicant's own checklist. This is advisory only — no automated accept/reject, and no biometric face-match between the ID and selfie; a reviewer still makes the actual call. Inspired by [fortnight-space](https://github.com/Lucienengolo/fortnight-space)'s document+selfie+extraction concept, but that project's actual verification (InsightFace face-matching, TrOCR text extraction) runs as local ML models via Streamlit/PyTorch — infra that doesn't run in a Deno edge function or a browser, so this adapts the concept rather than the code. Deployed and confirmed the auth gate + missing-secret graceful degradation work; the real Claude Vision call is untestable here until `ANTHROPIC_API_KEY` is set (same gap as Kairos Mind).
- 🔵 Reviewer/admin console UI — `profiles.is_reviewer` and the RLS to support one exist; no dashboard built yet. The new KYC verification columns (`instructor_credentials.verification_*`) are ready for this console to surface once it's built.
...
- ✅ **Full learner + instructor flow verified end-to-end against the live project**, with real accounts (not service-role bypasses) driving the exact REST calls the frontend makes: signed up + signed in a test student, self-enrolled in a free course, completed both lessons, passed the quiz, had the course marked complete and a certificate issued — then confirmed the certificate/dashboard read queries (with their joins) return correctly. Also signed in as a test instructor and confirmed they can list/manage their own course and add a lesson via their own RLS-scoped session (not just the earlier service-role seeding). Two negative security checks also passed: a student can't forge a certificate under a different `student_id` (RLS 42501), and can't edit a course they don't own (0 rows affected). Test accounts kept live per your request: `test-student-e2e@slearn-test.dev` / `test-instructor-e2e@slearn-test.dev` (password shared with you separately, not committed here since this repo is public); course "E2E Test Course — Intro to Testing".
...
- ✅ **Design audit against the prototypes + the design system's own tokens**, driven by `SLearn Design Review.dc.html` (pulled from the Design project, along with `SLearn Platform.dc.html` and the `current/`/`improved/` screen sets — these had never actually been committed, despite being referenced in this README's repo map). Confirmed and fixed every issue the review doc flagged, plus one it didn't:
  - **Header transparency bug** — `Header.tsx` toggled `bg-transparent` while scrolling up (and on initial load, since that was the default state), exactly as documented. Now permanently solid/blurred, matching `improved/*.dc.html`'s `.nav-wrap`.
  - **Leftover Tailwind blue** — `QuizViewer.tsx`, `AuthModal.tsx` (focus rings, submit button, link), plus undocumented instances in `App.tsx`/`CertificatesPage.tsx` (loading spinners), `HomePage.tsx` (hover states, icon badge, hero-banner text), and `StudentDashboard.tsx` (a badge) — all replaced with the design system's primary/gold tokens.
  - **Identical emoji-on-gradient course thumbnails** — replaced with a per-category gradient + Lucide icon (`lib/courseCovers.tsx`, matching the 5 seeded categories and the prototypes' own `covers` mapping) across `CourseCard.tsx`, `CourseDetail.tsx`, and `StudentDashboard.tsx`.
  - **Not in the review doc, found by reading the tokens directly**: the design system's own raw default maps `--color-primary` to green, but `improved/*.dc.html` explicitly overrides this to gold for S@Learn's actual yellow brand (scoped `.sl` class) — yet `tailwind.config.js` mirrored the *raw* default, so every primary CTA across the entire app (Sign In, Enroll Now, Continue Learning, quiz buttons, etc.) was rendering green instead of the intended brand gold. Remapped `primary` to the gold ramp. This also meant every `bg-primary-600 text-white` button needed a real second look, not just a hex swap: gold-600 with white text sits at ~4.45:1 contrast, just under AA's 4.5:1 for normal text. Fixed by moving to `bg-primary-500 text-gray-900` (dark-on-gold) throughout, matching the design system's own `--on-primary: var(--gray-900)` token — including two gradient banners (`CertificatesPage.tsx`, `StudentDashboard.tsx`) that also had light-tint body text (`text-blue-100`/`text-primary-100`) sitting directly on the gold gradient, which would have been unreadable regardless of the primary-color fix.
  - Confirmed via `tsc -b`, `eslint`, and a production build after each round of changes — no new errors, no regressions in existing lint findings (all remaining lint output is pre-existing, in the legacy `SLearn/` folder or already-documented `exhaustive-deps` warnings).
- 🟡 Port final prototype screens (`SLearn Platform.dc.html`) into the React components 1:1 — the audit above fixed concrete deviations; a full pixel-for-pixel port of the improved layouts (search-forward hero, sticky mobile enroll bar, Aria assistant launcher, etc.) is a larger follow-up, not done here.
...
- [ ] **Still needed from you** (I don't have these credentials and won't fabricate them): `ANTHROPIC_API_KEY`, `RESEND_API_KEY`, `STRIPE_SECRET_KEY`, and `STRIPE_WEBHOOK_SECRET` as function secrets (`supabase secrets set ANTHROPIC_API_KEY=... RESEND_API_KEY=... STRIPE_SECRET_KEY=... STRIPE_WEBHOOK_SECRET=...`), plus `VITE_CAL_BOOKING_LINK` in Vercel/`.env` (see §5.B) — without them, Kairos Mind, application/interview emails, and paid-course checkout will all error or no-op at runtime. For the Stripe webhook secret: create the webhook endpoint in the Stripe dashboard pointing at `https://hzcdcjtngxhxbzeirbmz.functions.supabase.co/stripe-webhook` (event: `checkout.session.completed`) first — Stripe generates the signing secret at that point.
  - **Cost reality check, since these three are easy to lump together as "paid stuff":** Stripe and Resend cost nothing to wire up — Stripe test-mode keys are free and only real successful charges take a fee, and Resend's free tier (3,000 emails/month) covers this app's volume many times over. Both just need you to sign up and hand me the key. Claude API access is the one that's genuinely metered from the first request — there's no free tier for API usage (unlike a chat subscription).
  - **Graceful degradation, fixed:** `kairos-mind-tutor` and `verify-identity-document` used to leak the raw Anthropic error text to users when the key is unset or out of credit (e.g. a chat bubble literally saying "Your credit balance is too low… Plans & Billing…"). Both edge functions now translate known failure modes (low credit, rate limit, overload) into plain, non-technical messages before they ever reach the client — Kairos Mind says it's "temporarily out of service," and KYC verification falls back to "a reviewer will check manually," matching the not-yet-configured case exactly.
  - **Free-tier alternative for Claude, if you want to keep building without spending on this yet (proposal, not implemented):** both edge functions are small, self-contained Anthropic SDK calls (a single `messages.create`/`messages.stream`). Google's Gemini API has an actual free tier for `gemini-2.0-flash`-class models with real rate limits, not a trial credit that runs out — swapping either function to it is a contained change (new SDK, different message/vision payload shape, no client-side changes needed) if you'd rather validate the features that way before paying for Claude. Not done here since picking a model provider is a real product decision, not something to switch silently.
