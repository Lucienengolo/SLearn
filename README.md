...
- [ ] **Still needed from you** (I don't have these credentials and won't fabricate them): `ANTHROPIC_API_KEY`, `RESEND_API_KEY`, `STRIPE_SECRET_KEY`, and `STRIPE_WEBHOOK_SECRET` as function secrets (`supabase secrets set ANTHROPIC_API_KEY=... RESEND_API_KEY=... STRIPE_SECRET_KEY=... STRIPE_WEBHOOK_SECRET=...`) — without them, Kairos Mind, application/interview emails, and paid-course checkout will all error at runtime. For the Stripe webhook secret specifically: create the webhook endpoint in the Stripe dashboard pointing at `https://hzcdcjtngxhxbzeirbmz.functions.supabase.co/stripe-webhook` (event: `checkout.session.completed`) first — Stripe generates the signing secret at that point.
