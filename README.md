...
- ✅ **Interview scheduling via Cal.com** — replaces the old bare `interview_slots` table (staff manually inserting rows, applicants picking from a fixed list with no calendar sync, video link, or reminders). Applicants now book through a real Cal.com event type; `supabase/functions/cal-webhook` (`BOOKING_CREATED`/`BOOKING_RESCHEDULED`/`BOOKING_CANCELLED`) creates/updates the `interviews` row and advances the application to `interview` status. `interview_slots` is left in the schema (not dropped) but the app no longer writes to it.
  - **Still needed from you**: a Cal.com account + event type for the interview, `VITE_CAL_BOOKING_LINK` (the event type's public booking URL — safe client-side) and `CAL_WEBHOOK_SECRET` (from Cal.com's webhook settings) as a function secret. Until both are set, the wizard/dashboard show a "not configured yet, we'll follow up by email" fallback instead of a broken link.
  - **Not yet verified against a real Cal.com webhook delivery** (unlike everything else built this session) — there's no live Cal.com account to test against here. What *was* verified end-to-end: deployed the function, set a temporary test secret, and drove all three event types against it with a synthetic HMAC-signed payload matching Cal.com's documented webhook shape — confirmed signature verification (valid/invalid/missing all behave correctly), `BOOKING_CREATED` (real profile lookup by attendee email → interview row created → application flipped to `interview` → notification queued), `BOOKING_RESCHEDULED` (`scheduled_at` updated), and `BOOKING_CANCELLED` (outcome marked `rescheduled`) all against a real in-progress application, then reverted it back to its original state afterward. Once you configure the real Cal.com webhook, check its first live delivery against the field names in `cal-webhook/index.ts` (`payload.uid`, `payload.attendees[].email`, `payload.videoCallData.url`) in case Cal.com's actual payload differs from its documented shape.
...
- [ ] **Still needed from you** (I don't have these credentials and won't fabricate them): `ANTHROPIC_API_KEY`, `RESEND_API_KEY`, `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, and `CAL_WEBHOOK_SECRET` as function secrets (`supabase secrets set ANTHROPIC_API_KEY=... RESEND_API_KEY=... STRIPE_SECRET_KEY=... STRIPE_WEBHOOK_SECRET=... CAL_WEBHOOK_SECRET=...`), plus `VITE_CAL_BOOKING_LINK` in Vercel/`.env` — without them, Kairos Mind, application/interview emails, paid-course checkout, and Cal.com interview scheduling will all error or no-op at runtime. For the Stripe webhook secret: create the webhook endpoint in the Stripe dashboard pointing at `https://hzcdcjtngxhxbzeirbmz.functions.supabase.co/stripe-webhook` (event: `checkout.session.completed`) first — Stripe generates the signing secret at that point. Same idea for Cal.com: create an event type, then a webhook (Cal.com dashboard → Settings → Developer → Webhooks) pointed at `https://hzcdcjtngxhxbzeirbmz.functions.supabase.co/cal-webhook` subscribed to `BOOKING_CREATED`/`BOOKING_RESCHEDULED`/`BOOKING_CANCELLED` — Cal.com generates the signing secret when you create it.
