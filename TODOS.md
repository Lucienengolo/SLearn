...
**`create-checkout-session` auth-check ordering (LOW).** The function checked `Authorization` header *presence* early, but didn't actually verify the token (`caller.auth.getUser()`) until after parsing the request body, checking the origin allowlist, and checking whether `STRIPE_SECRET_KEY` is configured -- so a request with a missing/garbage/expired token could still learn "this deployment has Payments not configured" (500) before ever being rejected as unauthenticated (401). Reordered so `auth.getUser()` runs immediately after the presence check, before any other work or config-revealing response. Found and fixed the identical copy-pasted pattern in `create-tutor-deposit-checkout` (the tutor-marketplace deposit-checkout sibling) at the same time, for the same reason. Not applied to other edge functions (`cancel-tutor-booking`, `instructor-approval`, etc.) -- checked each: none of them reveal deployment config ahead of the auth check, only benign body-shape validation, which isn't the same issue. **Deploy blocker, carried over from earlier in this session's roadmap:** committed and pushed, but not yet deployed to the Supabase Edge Functions runtime -- no `SUPABASE_ACCESS_TOKEN`/CLI login available this session (`supabase functions deploy` fails with `LegacyPlatformAuthRequiredError`), same credential gap noted for the FCFA currency fix in the Batch 7 entry below. `vercel.json`'s CSP fix has no such blocker (Vercel auto-deploys straight from the `main` push).
