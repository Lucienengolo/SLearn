...
## Fixed why tutor matching silently stopped working: acceptMatch() never set parent_timeout_at (2026-08-04)

Founder report: "the tutor matching is not working correctly." Investigated against real production data rather than guessing -- found only 1 of 3 verified instructors actually has `tutor_profile_fields` set up (a real prerequisite for candidacy, not itself a bug), and that tutor's *only* existing match (from 2026-07-30) has sat in `status = 'messaging'` for 5 days with no resolution. Since `ACTIVE_MATCH_STATUSES` in both `lib/matches.ts` and `match-tutor-request/index.ts` treats `'messaging'` as occupying the tutor, this one stuck match was silently blocking every subsequent match attempt for that tutor's subjects -- exactly reproduced by 7 fresh test requests the founder submitted today, all still `searching`.

The cron infrastructure meant to catch this already exists and is confirmed active in production (`select jobid, active from cron.job` -- all 4 tutor-marketplace timeout crons running hourly, `0031_tutor_marketplace_cron.sql`): `expire_unresponsive_parent_matches()` should auto-expire a `messaging` match after 48h of parent silence, freeing the tutor. It never fired here because its `WHERE parent_timeout_at is not null` clause never matched -- `lib/matches.ts`'s `acceptMatch()` (the function a tutor calls to accept a match) sets `status`/`tutor_responded_at`/`messaging_started_at` but never set `parent_timeout_at` at all. A real, previously-undiscovered gap: the column has existed since `0030_tutor_marketplace.sql`, the cron that depends on it has been scheduled and running since `0031`, but nothing ever populated it.

Fixed: `acceptMatch()` now sets `parent_timeout_at` to 48h out (matching the tutor-side `TUTOR_RESPONSE_WINDOW_MS` convention already used in the matching edge function). New `tests/matches.acceptMatch.test.ts` pins the actual regression (asserts `parent_timeout_at` is set and lands in the correct ~48h window, not just that the function runs). Full suite and typecheck clean.

**Still needed**: the one already-stuck production match (`265fd754-...`) has `parent_timeout_at = NULL` from before this fix and won't self-resolve -- the code fix only prevents this going forward. Asked the founder whether to manually expire it now (mirrors exactly what the cron would do) since a direct production data write was correctly blocked by the safety classifier pending explicit confirmation.

## Fixed the real bug behind "my CV isn't saved" and "I get pushed back to the start" -- both were the same root cause (2026-08-04)

Founder report: submitting a CV didn't seem to save, and finishing/leaving the application wizard threw the applicant back to the first step even though "the information remains on the different form." Investigated rather than assumed -- checked the storage bucket config (`instructor-credentials`: no MIME-type or size restriction, ruled out), the RLS policies on both `storage.objects` and `instructor_credentials` (correctly scoped to the applicant's own folder/application, ruled out), and the reviewer-side display (`ReviewQueue.tsx` already maps every credential generically with a working signed-URL download link, ruled out) -- the actual upload path and its visibility to reviewers were already correct.

**Real root cause**: `ApplicationWizard.tsx`'s step position (`useState(0)`) is pure local component state. `InstructorApplicationFlow` (the parent) correctly refetches the real saved application and credentials on every mount -- which happens on every page load, refresh, or navigation back to "Apply to teach" -- but a fresh mount always starts a brand-new component instance, so `step` unconditionally resets to 0 regardless of how far the applicant had actually gotten. The CV was never lost; the applicant just had to re-click through every prior step to see it again, which is exactly what "pushed back to the start" describes. Founder's own suggested fix (progress saved locally) was the right call: `step` is now persisted to `localStorage`, keyed per user id, restored via a lazy `useState` initializer on mount, written on every change, and cleared on successful submission (so a later "Edit information" re-entry into an already-submitted application starts clean rather than resuming a stale position).

No backend change needed -- this was purely a client-side state bug, not a data-loss or reviewer-visibility bug. `tests/ApplicationWizard.stepPersistence.test.tsx` (new, 4 tests) covers the actual regression: resumes at the last visited step after a real remount, starts fresh with no stored progress, keeps progress isolated per user, and clears on successful submission. `tests/ApplicationWizard.credentials.test.tsx`/`.tutoring.test.tsx` needed `localStorage.clear()` added to their `beforeEach` since they share the same mocked user id and would otherwise leak step state between tests. Full suite and typecheck clean.

## Instructor credential requirements rebalanced: selfie/degree/certificate optional, CV/sample-lesson compulsory, new question-paper-example credential added (2026-08-04)

Founder screenshots: an applicant was blocked from submitting by "Un selfie est requis avant de pouvoir soumettre" (the selfie-required check added earlier today) and asked for a full rebalance. **Supersedes this same file's earlier entry today**, which said the selfie "stays a required field" -- that was accurate when written, a few hours before this request. New rules: selfie/degree/certificate optional, CV/sample-lesson compulsory (neither had any requiredness enforcement before), and a brand-new **question paper example** credential, compulsory -- the founder's rationale: proves an applicant can write meaningful quizzes for students, not just teach.

New `question_paper` value on the `instructor_credential_type` Postgres enum (`0050_question_paper_credential_type.sql`) -- no RLS changes needed, the existing applicant/reviewer credential policies already cover any `credential_type` value, they don't enumerate specific ones. `ApplicationWizard.tsx`'s `handleSubmit` now blocks on missing government ID, CV, sample lesson, or question paper (in that order) and no longer blocks on a missing selfie. Every credential upload label now states its own requiredness directly (`"CV (required)"`, `"Degree (optional)"`, etc.) -- matches the existing convention `IdentityCapture.tsx` already used for the government-ID/selfie rows, extended to the other four. Added a `hasQuestionPaper` row to `VerificationPipeline.tsx`'s applicant-facing status checklist, mirroring the existing `hasSampleLesson` row. Fixed `lib/legalDocs.ts`'s Terms/Privacy wording again (a second pass today) -- it said "government ID + selfie (both required)", now correctly describes the selfie as optional-but-recommended.

New `tests/ApplicationWizard.credentials.test.tsx` (6 tests) covers the actual submission-blocking behavior end to end -- each required credential blocks independently in the right order, a full valid set with no selfie/degree/certificate submits successfully, and the upload-step labels read correctly. `tests/IdentityCapture.test.tsx` updated a second time today (selfie label went required -> optional). Full suite and typecheck clean.

## Two final pre-conclusion changes: live selfie capture removed, institutional pages show a maintenance notice (2026-08-04)

**Instructor verification: live camera selfie replaced with a file upload.** `IdentityCapture.tsx`'s selfie step used `navigator.mediaDevices.getUserMedia()` for an embedded live video feed + canvas capture. Same root cause as the earlier geolocation in-app-webview bug this session: `getUserMedia` is routinely blocked or unavailable in WhatsApp/Instagram-style in-app browsers, which is how a meaningful share of this app's users actually arrive. Now mirrors the government-ID upload exactly -- a plain file input with `capture="user"` (front-camera hint) instead of `"environment"`. The selfie stays a required field: `verify-identity-document` only ever processes `government_id` automatically, never the selfie (reviewers compare it to the ID visually) -- confirmed via the edge function code before touching anything, so removing the *live* mechanism doesn't lose any actual verification value. Updated the FR/EN strings (`lib/i18n.ts`) and the "live selfie"/"selfie en direct" references in `lib/legalDocs.ts`'s Terms/Privacy (both were describing a mechanism that no longer exists). `tests/IdentityCapture.test.tsx` updated to match.

**School/Business/Government pages: temporary maintenance notice.** Founder request, no reason given beyond "unavailable due to maintenance." New `lib/institutionalPagesConfig.ts` (`INSTITUTIONAL_PAGES_ENABLED`, false) gates `InstitutionalLandingPage.tsx` -- same kill-switch pattern as `PAYMENTS_ENABLED`, so it's independently mockable in tests and flipping it back on later is a one-line change, not a rewrite. The real page (value props, FAQ, inquiry form) is untouched underneath, just not rendered while the flag is off. No image asset was provided and there's no image-generation capability available -- built the notice from existing ink-and-paper design tokens (a `Construction` icon, `font-fraunces` heading, `text-warm-gray` body) rather than block on a custom graphic; founder confirmed this was fine over AskUserQuestion. `tests/InstitutionalLandingPage.test.tsx` now mocks the flag `true` to keep exercising the real page; new `tests/InstitutionalLandingPage.maintenanceGate.test.tsx` covers the actual (unmocked) default-disabled state for all 3 account types.

Full suite (404/404, 2 unrelated flaky timeouts on unmodified files reproduced as passing in isolation -- sandbox resource contention, not a regression) and typecheck clean.
...
**What's founder-run, not something this session could execute:** no Vercel CLI is installed in this environment and there's no active login, so the native WAF rules in `SECURITY.md` were handed over as documented commands rather than published directly -- consistent with Vercel's own guidance that rule *publishing* should always be a deliberate human action anyway (a bad rule can silently block the entire site). Attack Mode is also explicitly blocked for scripts/agents given its severity, by Vercel's own design, not a gap on this end.

**Status: both native WAF rules (exploit-probe path blocking, coarse rate-limit backstop) are published and live**, run and confirmed by the founder through the staged log -> preview -> production rollout. Walked the DDoS response plan section of `SECURITY.md` past the founder step by step (detection signals, escalation order, Attack Mode, postmortem template) -- confirmed it matches how they'd actually want to respond, not just filed away unread. Still open: `middleware.ts` remains in `LOG_ONLY` mode pending a representative period of real traffic review before enforcing.
...
1. **`quiz_questions.correct_answer` was readable by any student who could see the quiz at all.** `QuizViewer.tsx` did `select('*')` and graded client-side by comparing against `correct_answer` -- meaning the full answer key downloaded to the browser before a single question was answered. Anyone poking at the Network tab could read it.
2. **`quiz_attempts.score`/`total_points`/`passed` were plain client-submitted values, independent of #1** -- a direct API call could insert a fabricated `passed: true` attempt without ever fetching the real questions.
3. **`classwork_submissions.grade`/`feedback`/`graded_at`** (instructor-owned columns) had no restriction on the "students manage their own submissions" policy -- a student could self-grade their own assignment through the exact same policy that lets them submit it.

Also checked and ruled out as non-issues: `categories`' two INSERT policies looked like leftover duplicate cruft (one scoped to verified instructors, one wide open to any authenticated user) but the wide-open one is a deliberate founder decision from `0041_parents_create_categories.sql` (parents adding a custom subject when submitting a tutor request have no "verified" trust tier to check against) -- left alone. `interview_slots` has a real RLS gap (its booking policy doesn't identify who's booking) but grepping the client found zero references to the table -- interviews are booked through Cal.com per `supabase/functions/cal-webhook`, not this table, so there's no live exploit path; noted, not fixed.

Fixed in `0048_quiz_grading_and_classwork_hardening.sql`:
- Quiz grading moved fully server-side via `compute_quiz_grade()`, a `security definer` RPC that grades a submitted answer set against the real correct answers and returns only the result (score/passed/correct count) -- never the answers themselves. Called live by both guests (quiz preview, no account) and signed-in students for the on-screen result, and reused by a new `quiz_attempts` `BEFORE INSERT` trigger so the persisted record can't drift from what an honest call would have produced regardless of what a client claims. `quiz_questions` gets the same table-grant-replacement treatment as `profiles.email` (0046) to close the direct leak, with `get_quiz_questions_for_editing()` as the one RPC exception for the owning instructor's own quiz-editing flow (`CourseEditor.tsx`).
- `classwork_submissions` gets a guard trigger reverting `grade`/`feedback`/`graded_at` to their prior value unless the caller is the post's actual owning instructor (or `service_role`) -- same pattern as 0047's guard triggers, but checking real course ownership rather than just `service_role`, since grading here is a normal client-side instructor action.

Verified against `ci_test`: direct `correct_answer` select now denied outright; a fabricated `quiz_attempts` insert (score 999, passed true) gets silently corrected to the true grade computed from the real answers; the owning instructor's `get_quiz_questions_for_editing` call returns full rows including `correct_answer`, a non-owning instructor gets zero rows; a student's self-grade attempt on their own submission is silently reverted; the actual owning instructor's grade goes through normally. `QuizViewer.tsx` and `CourseEditor.tsx`'s `loadQuizDraft` updated to match (explicit column list / RPC call instead of `select('*')`); `QuizQuestion.correct_answer` becomes optional in `lib/supabase.ts`. Full suite and typecheck clean.
...
**Missing CSP (MEDIUM).** `vercel.json` had no `Content-Security-Policy` header at all -- the other headers (`X-Frame-Options`, `Permissions-Policy`, etc.) gave no defense-in-depth against script injection specifically. Added a real CSP scoped to what the app actually loads (checked, not guessed): `script-src 'self'` with no `unsafe-inline`/`unsafe-eval` (no inline `<script>` tags anywhere, including the PWA plugin's `registerSW.js`, which is a same-origin `src=` file, not inlined); `style-src`/`font-src` allow Google Fonts + Fontshare (the two font CDNs `index.html` actually links); `img-src`/`media-src`/`connect-src` scoped to `'self'` + the Supabase project domain (avatars/thumbnails/lesson video are all Supabase Storage URLs, confirmed by grep -- no other image/media host is used); `frame-src` allows YouTube/Vimeo only (the lesson `video_url` embed field's documented purpose); `object-src 'none'`, `frame-ancestors 'none'`, `base-uri 'self'`, `form-action 'self'` for the rest. PostHog/Sentry aren't in `connect-src` since neither is configured in this deployment (both are no-ops without a key/DSN) -- if either gets configured later, `connect-src` needs a matching addition or that integration will silently fail to send data under this CSP. Verified: production build's `dist/index.html` has zero inline scripts, `npm run build`/typecheck clean.

**`create-checkout-session` auth-check ordering (LOW).** The function checked `Authorization` header *presence* early, but didn't actually verify the token (`caller.auth.getUser()`) until after parsing the request body, checking the origin allowlist, and checking whether `STRIPE_SECRET_KEY` is configured -- so a request with a missing/garbage/expired token could still learn "this deployment has Payments not configured" (500) before ever being rejected as unauthenticated (401). Reordered so `auth.getUser()` runs immediately after the presence check, before any other work or config-revealing response. Found and fixed the identical copy-pasted pattern in `create-tutor-deposit-checkout` (the tutor-marketplace deposit-checkout sibling) at the same time, for the same reason. Not applied to other edge functions (`cancel-tutor-booking`, `instructor-approval`, etc.) -- checked each: none of them reveal deployment config ahead of the auth check, only benign body-shape validation, which isn't the same issue. Deployed to the Supabase Edge Functions runtime (founder generated a fresh personal access token this session to unblock `supabase login`, resolving the credential gap noted for the FCFA currency fix in the Batch 7 entry below -- `supabase functions deploy <name> --use-api --import-map supabase/functions/deno.json`, the `--import-map` flag needed explicitly since the no-Docker `--use-api` bundling path didn't auto-discover `deno.json`'s `stripe`/`@supabase/supabase-js` import-map entries on its own). Live-verified: an invalid bearer token is rejected with 401 before reaching the function at all (Supabase's gateway-level JWT verification, a stronger first line of defense than the finding assumed) -- the in-function reorder is now correctly-layered defense-in-depth for the narrower case of a gateway-valid-but-otherwise-invalid session.

## Security review (pre-beta): profiles.email was publicly readable by anyone, view-based fix (2026-08-02)

Pre-beta pentest (read-only RLS probes + live edge-function calls, scoped via AskUserQuestion) found `profiles`' "publicly readable" RLS policy (`using (true)`, meant to expose `full_name`/`role`/`verified`/`avatar_url` for course/review/tutor-match attribution) has no column restriction -- RLS filters rows, not columns. A plain unauthenticated `select=*` against production returned every registered user's `email`, confirmed live via curl. Fixed with `0046_restrict_profile_email.sql`: revoked the table-level `SELECT` grant on `profiles` from `anon`/`authenticated` entirely (a column-level revoke alone is a no-op against Supabase's default broad table-level grant -- verified this the hard way against the local `ci_test` stub before it could reach production) and replaced it with an explicit column list omitting `email`, plus a `public_profiles` view carrying the same safe columns. Email was genuinely needed in 3 existing relationships beyond "my own row" (instructor -> their enrolled students, reviewer -> applicant/course-instructor under moderation, "add student by email"), each now served by its own security-definer RPC that independently re-verifies the caller's relationship server-side rather than trusting the client -- the last of these (`find_student_id_by_email`) is a real tightening besides, since the old direct lookup had no relationship check at all. Every bare `profiles` `select('*')` client-side (own profile in `AuthContext`, tutor profile in `lib/matches.ts`, two `head:true` count queries) needed a matching fix, since Postgres's `*` expansion checks column privilege before RLS and hard-errors entirely rather than silently omitting the restricted column. Verified: full suite (397/397), typecheck clean, migration deployed to production, live curl re-probe confirms `email` is no longer returned.
